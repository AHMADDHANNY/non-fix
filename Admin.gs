  ...
  * =================================================================== */
  ...
  function editUser(
    ...
    try {
      ...
      // Admin yang sedang login tidak boleh mengubah role dirinya sendiri.
      if (
        idUser === String(session.user.id_user) &&
        role !== 'admin'
      ) {
        throw new Error(
          'Anda tidak dapat mengubah role akun admin yang sedang digunakan.'
        );
      }
      ...
      var roleLama = String(
        data[rowIndex][KOLOM_USERS_ADMIN.ROLE] || ''
      ).toLowerCase();

      var statusLama = String(
        data[rowIndex][KOLOM_USERS_ADMIN.STATUS_AKTIF] || ''
      ).toLowerCase();

      // Jangan sampai sistem kehilangan seluruh admin aktif.
      if (
        roleLama === 'admin' &&
        statusLama === 'aktif' &&
        (role !== 'admin' || statusAktif !== 'aktif')
      ) {
        var jumlahAdminAktif = 0;

        for (var a = 1; a < data.length; a++) {
          var roleCheck = String(
            data[a][KOLOM_USERS_ADMIN.ROLE] || ''
          ).toLowerCase();

          var statusCheck = String(
            data[a][KOLOM_USERS_ADMIN.STATUS_AKTIF] || ''
          ).toLowerCase();
        ...
        if (jumlahAdminAktif <= 1) {
          throw new Error(
            'Admin terakhir tidak dapat diubah menjadi pegawai atau dinonaktifkan.'
          );
        }
      }
  ...
  * =================================================================== */
  ...
  function hapusUser(token, idUser) {
    ...
    try {
      ...
      if (statusTarget !== 'aktif') {
        throw new Error('User tersebut sudah nonaktif.');
      }
      ...
      if (roleTarget === 'admin') {
        ...
        if (jumlahAdminAktif <= 1) {
          throw new Error(
            'Admin terakhir tidak dapat dinonaktifkan. Tambahkan admin lain terlebih dahulu.'
          );
        }
      }
  ...
  * =================================================================== */
  ...
  function simpanPengaturan(
    ...
    try {
      ...
      var menitMasuk =
        parseInt(jamMasuk.substring(0, 2), 10) * 60 +
        parseInt(jamMasuk.substring(3, 5), 10);

      var menitPulang =
        parseInt(jamPulang.substring(0, 2), 10) * 60 +
        parseInt(jamPulang.substring(3, 5), 10);
  ...
  * =================================================================== */
   ...
   * }
   */
  function _sanitizeNamaFileAdmin_(text) {
    return String(text || '')
      .replace(/[\\/:*?"<>|#%{}~&]/g, '_')
      .replace(/\s+/g, '_')
      .substring(0, 100);
  ...
  function exportRekapAbsensiPDF(
    ...
    try {
      ...
      var fileName =
        'Rekap_Absensi_' +
        _sanitizeNamaFileAdmin_(namaPeriode) +
        _sanitizeNamaFileAdmin_(namaUser) +
        '_' +
  ...
  /* ===================================================================
  * 11. DOWNLOAD PDF SECARA AMAN
  * =================================================================== */

  /**
   * Mengambil PDF milik aplikasi dalam bentuk Base64.
   *
   * Dipakai frontend setelah exportRekapAbsensiPDF() berhasil.
   * File tetap private di Google Drive sehingga tidak perlu
   * menggunakan ANYONE_WITH_LINK.
   */
  function downloadRekapAbsensiPDF(token, fileId) {
    try {
      _verifyAdmin_(token);
      ...
      if (!fileId) {
        throw new Error('ID file PDF wajib diisi.');
      }

      var file = DriveApp.getFileById(fileId);

      if (file.isTrashed()) {
        throw new Error('File PDF sudah tidak tersedia.');
      }
